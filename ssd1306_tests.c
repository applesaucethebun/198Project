...
void ssd1306_PrintNumbers() {
    char numberStr[2]; // Buffer to hold the string representation of numbers

    for (int i = 1; i <= 9; i++) {
        ssd1306_Fill(Black); // Clear the display
        snprintf(numberStr, sizeof(numberStr), "%d", i); // Convert number to string
        ssd1306_SetCursor(2, 0); // Set cursor position (2 is an example x offset)
        ssd1306_WriteString(numberStr, Font_11x18, White); // Display the number
        ssd1306_UpdateScreen(); // Refresh the screen to show changes
        HAL_Delay(1000); // Delay for 1 second (adjust the delay as needed)
    }
}

void ssd1306_ScrollText(const char* text) {
    ssd1306_Fill(Black); // Clear the display
    uint8_t length = strlen(text);
    uint8_t x = SSD1306_WIDTH;

    while (x > -length * 6) { // Adjust the 6 multiplier based on your font width
        ssd1306_Fill(Black); // Clear the display
        ssd1306_SetCursor(x, 0);
        ssd1306_WriteString(text, Font_6x8, White);
        ssd1306_UpdateScreen();
        HAL_Delay(50); // Delay to control scroll speed
        x--;
    }
}

void ssd1306_BouncingBall() {
    ssd1306_Fill(Black);
    int x = 10, y = 10;
    int8_t x_speed = 1, y_speed = 1;
    int radius = 4;

    for (int i = 0; i < 500; i++) { // Arbitrary number of iterations
        ssd1306_Fill(Black);
        ...
        if (x - radius <= 0 || x + radius >= SSD1306_WIDTH) {
            x_speed = -x_speed; // Reverse direction
        }
        if (y - radius <= 0 || y + radius >= SSD1306_HEIGHT) {
...
void ssd1306_ProgressBar() {
    ssd1306_Fill(Black);
    for (int progress = 0; progress <= SSD1306_WIDTH; progress++) {
        ssd1306_Fill(Black);
        ssd1306_DrawRectangle(2, 20, SSD1306_WIDTH - 4, 40, White); // Outline
        ssd1306_FillRectangle(3, 21, 3 + progress, 39, White); // Progress bar fill
        ssd1306_SetCursor(40, 0);
        char buffer[20];
        snprintf(buffer, sizeof(buffer), "Progress: %d%%", (progress * 100) / SSD1306_WIDTH);
        ssd1306_WriteString(buffer, Font_6x8, White);
        ssd1306_UpdateScreen();
        HAL_Delay(50); // Control the speed of progress
    }
}

void ssd1306_BlinkingText(const char* text) {
    for (int i = 0; i < 10; i++) { // Number of blinks
        ssd1306_Fill(Black);
...
void ssd1306_SimpleAnimation() {
    int x = 0;
    while (x < SSD1306_WIDTH - 10) {
        ssd1306_Fill(Black);
        ssd1306_DrawRectangle(x, 20, x + 10, 30, White); // Simple square as a "sprite"
        ssd1306_UpdateScreen();
        x++;
        HAL_Delay(20);
    }
